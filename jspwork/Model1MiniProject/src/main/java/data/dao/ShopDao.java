...
public class ShopDao {
	  ...
	  //장바구니 담을 때 멤버테이블의 num을 넣어야하는데 세션에 아이디가 있으므로 아이디를 이용해서 num을 얻는 메서드 추가
      ...
      //cart  //// key값은 string만 됨. value는 string int 타입 가능
      public List<HashMap<String, String>> getCartList(String id) {
    	  ...
    	  try {
			...
			while(rs.next()) {
				...
				map.put("cartday", rs.getString("cartday")); //type을 string으로 하려고했으므로 timestamp 안해줌
